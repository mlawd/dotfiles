...
description:
  Use when analysing unresolved PR review comments across a Graphite stack.
  Discovers the stack, loads unresolved review threads for each PR, analyses each
  comment for correctness and impact, and categorises it as "needs fix" or
  "dismiss" with a proposed action. Analyses only -- never implements.
---

# Comment Analyser

Analyse unresolved PR review comments across a Graphite stack and produce a
per-branch breakdown that categorises each thread as **needs fix** or
...
Run `gt log short` to ascertain which branches are in the current stack. For each
branch, run `gh pr list --head {branch} --json number` to get the PR number.
...
- Consider the whole stack -- a comment on one branch may be addressed by changes
  in another branch.
...
Present findings grouped by branch (bottom of stack first). For each branch, list
every unresolved thread with:
